...
def draw_networkx_edges(
    ...
    **kwargs):
    ...
    if chart is None:
        ...
        if url is not None:
            df_edges.to_json(url + '_edges.json', orient='records')
            edge_chart = alt.Chart(url + '_edges.json')
        else:
    ...
    edge_chart = edge_chart.mark_line(**marker_attrs).encode(
        x=alt.X('x:Q', axis=alt.Axis(title='', grid=False, labels=False, ticks=False), scale=alt.Scale(domain=(-1.1, 1.1))),
        y=alt.Y('y:Q', axis=alt.Axis(title='', grid=False, labels=False, ticks=False), scale=alt.Scale(domain=(-1.1, 1.1))),
        detail='edge:N',
        **encoded_attrs
    )
...
def draw_networkx_arrows(
    ...
    **kwargs):
    ...
    # Draw edges
    edge_chart = edge_chart.mark_text(
        **marker_attrs
    ).encode(
        x=alt.X('x:Q', axis=alt.Axis(grid=False, labels=False, ticks=False), scale=alt.Scale(domain=(-1.1, 1.1))),
        y=alt.Y('y:Q', axis=alt.Axis(grid=False, labels=False, ticks=False), scale=alt.Scale(domain=(-1.1, 1.1))),
        detail='edge',
        size = 's:Q',
        **encoded_attrs
    )
...
def draw_networkx_nodes(
    ...
    **kwargs
    ...
    if layer is not None:
        ...
        if url is not None:
            df_nodes.to_json(url + '_nodes.json', orient='records')
            node_chart = alt.Chart(url+ '_nodes.json')
        else:
